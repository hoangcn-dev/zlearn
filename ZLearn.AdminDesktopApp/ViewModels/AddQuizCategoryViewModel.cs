...
{
    public class AddQuizCategoryViewModel : ViewModelBase
    {
        ...
        public AddQuizCategoryViewModel(
            VariableStore store,
        ...
        private async Task AddCateAsync()
        {
            ...
            var res = await ExecuteAsync(() => _quizApiService.CreateNewCategory(CategoryName));
            if (res is not null && res.Succeeded)
            {
                MessageBox.Show("Tạo danh mục mới thành công");
                _windowManager.CloseWindow<AddQuizCategoryWindow>();
                return;
            }
            else
            {
